...
This module statically checks function and method complexities against LLMTestbed's architecture constraints.
    ...
    for file_path in py_files:
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                if node.name in legacy_exclusions:
                    continue
                ...
                if logical_count > max_allowed_lines:
                    ...
                    violations.append(
                        f"[{rel_path}:{node.lineno}] '{func_name}' has {logical_count} logical lines (Max allowed is {max_allowed_lines})"
                    )

    if violations:
        error_msg = "\n".join(violations)
        pytest.fail(
            f"Style Guide Violation: The following functions/methods exceed the 20-line logic limit:\n{error_msg}"
        )
...
def test_enforce_class_layout_ordering():
    ...
    2. Public API (methods without a leading single underscore, plus double-underscored magic methods) second.
    ...
    for file_path in py_files:
        ...
        for node in ast.walk(tree):
            ...
            # 2: Private Internal Helpers (leading single underscore, e.g. starts with '_' but not '__')
            current_state = 0
            ...
            for child in node.body:
                ...
                if isinstance(child, (ast.Assign, ast.AnnAssign)):
                    if current_state > 0:
                        rel_path = os.path.relpath(file_path, os.path.dirname(src_dir))
                        violations.append(
                            f"[{rel_path}:{child.lineno}] Class '{node.name}' has class variable after method definitions"
                        )
                # 2. Check for Methods
                elif isinstance(child, (ast.FunctionDef, ast.AsyncFunctionDef)):
                    method_name = child.name
                    # Determine category:
                    # Double underscore methods like __init__ or __call__ are considered Public API / setup
                    if method_name.startswith("_") and not method_name.startswith("__"):
                        ...
                        if current_state > 1:
                            rel_path = os.path.relpath(file_path, os.path.dirname(src_dir))
                            violations.append(
                                f"[{rel_path}:{child.lineno}] Class '{node.name}' has public method '{method_name}' defined after private helper method(s)"
                            )
...
def test_bandit_security_scan():
    ...
    result = subprocess.run(["bandit", "-r", "src", "-q"], capture_output=True, text=True)
    if result.returncode != 0:
...
def test_radon_complexity_scan():
    ...
    result = subprocess.run(["radon", "cc", "src", "-s"], capture_output=True, text=True)
    if result.returncode != 0:
...
def test_pylint_linting():
    ...
    result = subprocess.run(["pylint", "src", "--errors-only"], capture_output=True, text=True)
    if result.returncode != 0:
