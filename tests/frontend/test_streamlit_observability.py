...
def test_observability_processor_security_status():
    ...
    # 1. Safe status
    score_p, summary, badge = proc.get_security_status({"risk_score": 0.12, "summary": "Safe content"})
    assert score_p == 12.0
    assert summary == "Safe content"
    assert badge == "green"
    
    # 2. Unsafe (High Risk) status
    score_p, summary, badge = proc.get_security_status({"risk_score": 0.85, "summary": "Prompt Injection Attempt"})
    assert score_p == 85.0
    assert badge == "red"
    
    # 3. Neutral status
    score_p, summary, badge = proc.get_security_status({"risk_score": 0.40, "summary": "Neutral content"})
    assert score_p == 40.0
    assert badge == "orange"
